...
async function checkExtensionUpdates(projectDir) {
  const registryPath = path.join(projectDir, '.scofield', 'extensions', '_extensions.json');
  if (!fs.existsSync(registryPath)) return;

  const registry = JSON.parse(fs.readFileSync(registryPath, 'utf8'));
  if (!registry.extensions || registry.extensions.length === 0) return;
  ...
  for (const ext of registry.extensions) {
    const [owner, repo] = ext.source.split('/');
      ...
      https.get(url, (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            if (remoteVersion && remoteVersion !== ext.version) {
              console.log(`  ~ ${ext.name}: v${ext.version} → v${remoteVersion} available`);
              console.log(`    Run: scofield extension update ${ext.name}`);
            } else {
              console.log(`  = ${ext.name} v${ext.version} is up to date`);
            }
          } catch {
            console.log(`  ? ${ext.name}: could not check remote version`);
          }
          resolve();
        });
      }).on('error', () => {
        console.log(`  ? ${ext.name}: could not reach ${ext.source}`);
        resolve();
      });
    });
  }
...
module.exports = async function update(args) {
  ...
  // 6. Check installed extensions for available updates
  await checkExtensionUpdates(projectDir);

  // 7. Display update summary from CHANGELOG
