...
**UI Kit** — The mandatory design system source of truth for a Scofield project, living at `specs/ui/UI_KIT.md`. Contains all tokens, typography definitions, color values, spacing scales, and component specs used in the project. Items in the UI Kit have a `status` field (`pending` or `approved`). The UI Kit is visualized as an HTML artifact generated by the agent from this file — the artifact is always regenerated from the file and never holds its own state.
